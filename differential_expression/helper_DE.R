...
## Calculate tpr & fpr
calculate_stats <- function(called_lst, truth, N_genes, N_DE){
  tpr <- length(intersect(called_lst, truth)) / N_DE
  fpr <- length(setdiff(called_lst, truth)) / (N_genes - N_DE)
  return(c(tpr=tpr, fpr=fpr))
}
  
## Use sampling to estimate ensembled standard error of coefficient beta
estimate_std <- function(beta_mat, std_mat, weights_seq, B=1000){
  std_seq <- rep(NA, nrow(beta_mat))
  for(r in 1:nrow(beta_mat)){
    tmp_beta <- beta_mat[r, ]; tmp_std <- std_mat[r, ]
    e_sample <- rep(NA, B)
    for(bb in 1:B){
      sample_beta_seq <- mapply(function(t_beta, t_std){
        rnorm(1, mean=t_beta, sd=t_std)
      }, tmp_beta, tmp_std)
      e_sample[bb] <- sample_beta_seq %*% weights_seq
    }
    std_seq[r] <- sd(e_sample)# / sqrt(B)
  }
  return(std_seq)
}
...
ensmbl_ttest <- function(ensmbl_beta, ensmbl_std, ensmbl_df){
  ensmbl_tvalue <- ensmbl_beta / ensmbl_std
  ensmbl_pvalue <- 2 * pt(abs(ensmbl_tvalue), lower.tail=FALSE, df=ensmbl_df)
  return(ensmbl_pvalue)
}
  

## Ensemble functions
# (batch size weighted) averaging
ensmbl_DE_avg <- function(beta_mat, std_mat, weights_seq){
  # ensembled coefficient
  ensmbl_beta <- as.numeric(beta_mat %*% weights_seq)
  # use sampling to estimate ensembled standard error
  ensmbl_std <- estimate_std(beta_mat, std_mat, weights_seq) #rowMeans(std_mat)
  # calcualte ensembled p value based on ensembled beta and standard error
  ensmbl_pvalues <- ensmbl_ttest(ensmbl_beta, ensmbl_std, ensmbl_df=1)
  return(ensmbl_pvalues)
}
