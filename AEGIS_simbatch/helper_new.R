...
predLasso_pp <- function(trn_set, tst_set, y_trn, ...){
  mod <- glmnet::cv.glmnet(x=t(trn_set), y=as.numeric(as.character(y_trn)), ...)
  pred_trn_prob <- as.vector(predict(mod, newx=t(trn_set), s="lambda.1se"))
  pred_tst_prob <- as.vector(predict(mod, newx=t(tst_set), s="lambda.1se"))
