...
if(demo){
  ...
  setwd("/restricted/projectnb/combat/work/yuqingz/multistudy_batcheffect/newTB")
  data_dir <- "."
  script_dir <- "."
}
sapply(c("glmnet", "SummarizedExperiment", "sva", "DESeq2", "ROCR", "ggplot2", 
         "gridExtra", "reshape2", "dplyr", "nnls"), require, character.only=TRUE)
load(file.path(data_dir, "new_combined_unmatched.RData"))
...
for(s in study_names){
  ...
  batch <- rep(c(1, 2), times=sapply(dat_lst[train_name], ncol))
  ...
  for(l_type in learner_types){
    ...
    pred_unadj_res <- trainPipe(train_set=dat_whole_norm, train_label=group, 
                                test_set=NULL, 
    ...
    pred_combat_res <- trainPipe(train_set=dat_combat_norm, train_label=group, 
                                 test_set=NULL, 
                                 lfit=learner_fit, use_ref_combat=use_ref_combat)
