...
CS_zmatrix <- function(study_lst, label_lst, lfit, perf_name, use_ref_combat){
  ...
  for(i in 1:n_batch){
    for(j in 1:n_batch){
      if(i!=j){
        tmp_pred <- trainPipe(train_set=study_lst[[i]], train_label=label_lst[[i]], test_set=study_lst[[j]], 
...
Reg_SSL_pred <- function(study_lst, label_lst, lfit, use_ref_combat){
  ...
  for(k in 1:n_batch){
    tmp <- lapply(1:n_batch, function(batch_id){
      return(trainPipe(train_set=study_lst[[batch_id]], train_label=label_lst[[batch_id]], test_set=study_lst[[k]], 
                       lfit=lfit, use_ref_combat=use_ref_combat)$pred_tst_prob)
    })
